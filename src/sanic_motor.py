...
class BaseModel:
    ...
    def get_page_args(cls, request=None, page_name='page',
        ...
        if request:
            if not page:
                page = request.get(page_name, 1)

            if not per_page:
                per_page = request.get(per_page_name, 10)
    ...
    async def find(cls, request=None, *args, **kwargs):
        ...
        _, per_page, skip = cls.get_page_args(request, page_name,
        ...
        cur = coll.find(request, *args, **kwargs)
        objs = []
        if do_async_for:
            if as_raw:
